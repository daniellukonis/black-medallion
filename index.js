...
function fillCircle(color = "#FFF", ratio = 0.95){
    const canvas = document.querySelector('canvas')
    const context = canvas.getContext('2d')
...
class Blade{
    constructor(color = "#000", ratio = 0.9){
        ...
        this.circAngleDeg = Math.round(this.circAngle * 180 / Math.PI)
...
fillBackground()
fillCircle()
fillCircle("#000",0.025)

const blade1 = new Blade("#000")
blade1.drawBlades()

window.$fxhashFeatures = {
    "Radius Count": blade1.radiusRotations,
    "Circumference Count": blade1.circRotations,
    "Starting Angle (degrees)": blade1.circAngleDeg
}